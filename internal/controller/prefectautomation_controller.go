...
func (r *PrefectAutomationReconciler) syncWithPrefect(ctx context.Context, automation *prefectiov1.PrefectAutomation) (ctrl.Result, error) {
	...
	if automation.Status.Id != nil && *automation.Status.Id != "" {
		// Skip the PUT when the remote already matches, and preserve the
		// server-assigned trigger id when it doesn't: a trigger sent without an
		// id gets a fresh UUID, and proactive-trigger state buckets are keyed by
		// that UUID — every rotation orphans them, so a trigger whose `within`
		// exceeds the resync interval can never fire.
