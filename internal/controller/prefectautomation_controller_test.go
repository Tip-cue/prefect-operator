...
var _ = Describe("PrefectAutomation controller", func() {
	...
	Context("When the drift resync fires with no changes", func() {
		It("Should skip the Prefect update entirely", func() {
			...
			// The remote already matches, so no PUT happened: an update without a
			// trigger id makes the server rotate the trigger UUID, orphaning the
			// trigger's proactive state buckets.
